# 📘 GUIA DE INTEGRAÇÃO - Google Apps Script

## ⚠️ IMPORTANTE

**NÃO APAGUE** seu código existente! Você vai **ADICIONAR** as novas funções ao código que já tem.

---

## 🔧 OPÇÃO 1: Integração Manual (RECOMENDADO)

### Passo 1: Abra seu Google Apps Script atual
1. Google Sheets > Extensões > Apps Script
2. **NÃO APAGUE NADA AINDA**

### Passo 2: Adicione as funções auxiliares no TOPO do arquivo

Cole isto logo **DEPOIS** das suas variáveis/constantes existentes (RELEVANCIA, etc.):
...
### Passo 3: Adicione as funções do DIÁRIO

Cole isto em qualquer lugar ANTES do `doGet()`:
...
### Passo 4: Adicione as funções do DATA ENTRY

Cole isto logo após as funções do DIÁRIO:
...
### Passo 5: Modifique seu doGet() existente

Encontre sua função `doGet(e)` e **ADICIONE** estas linhas no **INÍCIO** da função:
...
function doGet(e) {
  ...
  // ⬆️ FIM DAS LINHAS ADICIONADAS ⬆️

  // ... seu código existente continua aqui ...
}
```

### Passo 6: Modifique seu doPost() existente

Encontre sua função `doPost(e)` e **ADICIONE** estas linhas no **INÍCIO** da função:
...
### Passo 7: Salve e Deploy

1. Salve tudo (Ctrl+S)
2. Implantar > Gerenciar implantações
3. Editar implantação ativa
4. Nova versão
5. Implantar

---

## 🔧 OPÇÃO 2: Substituição Completa (APENAS se não tiver outras funcionalidades)

Se você **NÃO TIVER** outras funcionalidades além de DIÁRIO e DATA ENTRY, pode usar:

📄 **[GOOGLE_APPS_SCRIPT_SIMPLES.js](GOOGLE_APPS_SCRIPT_SIMPLES.js)**
...
1. **Planejamento** → Deve salvar no **DIÁRIO**
2. **Registro** → Deve salvar no **DATA ENTRY**
3. **Outras funcionalidades** → Devem continuar funcionando normalmente
...
1. Veja os logs: **Execuções > Ver logs**
2. Procure por erros vermelhos
3. Verifique se não apagou nenhuma função existente por acidente
